...
// Optional: You can include a fallback logo or pass logos via match object
const placeholderLogo = "https://via.placeholder.com/50";

export default function MatchCard({ match, onSelect }) {
  // Prevent crashing if match is undefined
  if (!match) return null;

  const { id, team1, team2, odds, team1Logo, team2Logo } = match;

  return (
    <div
      className="bg-white rounded-2xl p-6 shadow-lg border border-blue-200 transition-all duration-300 hover:scale-105 hover:shadow-xl cursor-pointer max-w-md mx-auto"
